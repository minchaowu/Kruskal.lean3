import dickson data.set.function
...
by_cases
(λ pos : b = 0, by simp [pos, zero_le])
(λ neg : b ≠ 0, 
 have ∃ k, b = succ k, from exists_eq_succ_of_ne_zero neg,
 let ⟨k,hk⟩ := this in
 have pred (succ k) < a, by rw hk at H;exact H,
 have k < a, by rw pred_succ at this; exact this,
...
have h : b ≤ a, from le_of_lt H1,
have b - c ≤ a - c, from nat.sub_le_sub_right h c,
or.elim (lt_or_eq_of_le this)
(λ Hl, Hl)
(λ Hr, 
 have a = b, by rw [←eq₁, ←eq₂, Hr], 
 have refl : a > a, begin rw [←this] at H1, exact H1 end,
 absurd refl (lt_irrefl a))

theorem lt_pred_nonzero_self {n : ℕ} (H : n ≠ 0) : pred n < n :=
have ∃ k, n = succ k, from exists_eq_succ_of_ne_zero H,
let ⟨k,hk⟩ := this in begin simp [hk, pred_lt, lt_succ_self] end
